...
def _cached_searoute(origin_key: tuple, dest_key: tuple):
    """Memoized wrapper around searoute's Dijkstra query. Keys are (lon, lat)
    tuples rounded to 3 decimals by the caller so repeated port-pair traffic
    (the common case) hits cache instead of re-running the graph search."""
    return _sr.searoute(origin_key, dest_key, append_orig_dest=True)

def maritime_route_points(
    origin_lat: float, origin_lon: float,
    dest_lat: float, dest_lon: float,
    segments: int = 64,
) -> tuple[List[List[float]], str]:
    """
    Water-following track between two points, for map rendering and zone
    screening. Falls back to the great-circle geodesic (great_circle_points)
    when searoute is unavailable or cannot resolve a sea path (e.g. degenerate
    or landlocked inputs). Returns (points, source) so callers can surface
    which strategy produced the track.
    """
    if _USE_SEAROUTE:
        try:
            feature = _cached_searoute(
            ...
            if len(coords) >= 2:
                ...
                return _resample(points, segments), "searoute"
        except Exception:
            pass  # any network/graph failure falls through to great-circle
    return great_circle_points(origin_lat, origin_lon, dest_lat, dest_lon, segments), "great_circle_fallback"

def _resample(points: List[List[float]], target_count: int) -> List[List[float]]:
    """Evenly resample a polyline to ~target_count points so payload size stays
    bounded regardless of how many waypoints searoute's Dijkstra path returns."""
    if len(points) <= target_count:
        return points
    step = (len(points) - 1) / target_count
    return [points[round(i * step)] for i in range(target_count + 1)]